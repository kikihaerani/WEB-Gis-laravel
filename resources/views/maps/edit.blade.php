          ...
          <form class="mt-8 space-y-6" action="/maps/{{ $map->id }}" method="post">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
              <div>
                ...
                <input id="nama" name="provinsi" type="text" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Provinsi" value="{{ $map->provinsi }}">
                @error('Nama')
              ...
              <div>
                ...
                <input id="kategori" name="kategori" type="text" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Kota/Kab"  value="{{ $map->kotaorkab }}">
                @error('kategori')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
              </div>
